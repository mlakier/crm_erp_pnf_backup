...
const STORE_PATH = path.join(process.cwd(), 'config', 'vendor-refund-detail-customization.json')

function cloneDefaults(): VendorRefundDetailCustomizationConfig {
  return JSON.parse(JSON.stringify(defaultVendorRefundDetailCustomization())) as VendorRefundDetailCustomizationConfig
}
...
function normalizeGlImpactColumns(
  input: unknown,
  fallback: VendorRefundDetailCustomizationConfig['glImpactColumns'],
): VendorRefundDetailCustomizationConfig['glImpactColumns'] {
  const overrides =
    input && typeof input === 'object'
      ? (input as Partial<
          Record<
            TransactionGlImpactColumnKey,
            Partial<VendorRefundDetailCustomizationConfig['glImpactColumns'][TransactionGlImpactColumnKey]>
          >
        >)
      : {}
  ...
  ) as VendorRefundDetailCustomizationConfig['glImpactColumns']
}

export async function loadVendorRefundDetailCustomization(): Promise<VendorRefundDetailCustomizationConfig> {
  try {
    const raw = await fs.readFile(STORE_PATH, 'utf8')
    const defaults = cloneDefaults()
    const parsed = JSON.parse(raw) as Partial<VendorRefundDetailCustomizationConfig>
    return {
      ...defaults,
      ...parsed,
...
export async function saveVendorRefundDetailCustomization(
  nextConfig: VendorRefundDetailCustomizationConfig,
): Promise<VendorRefundDetailCustomizationConfig> {
  const defaults = cloneDefaults()
  const normalized = {
    ...defaults,
    ...nextConfig,
