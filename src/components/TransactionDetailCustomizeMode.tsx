...
type LayoutConfig = {
  ...
  lineColumns?: Record<string, { visible: boolean; order: number }>
...
export default function TransactionDetailCustomizeMode({
  detailHref,
  initialLayout,
  fields,
  saveEndpoint,
  emptyCellLabel = 'Select field...',
  introText = 'Edit the detail layout in context. Each filled box is a field placement, and empty boxes are open grid cells.',
  sectionDescriptions,
  statCardDefinitions,
  lineColumnDefinitions,
  statCardsTitle = 'Stat Cards',
  statCardsIntro = 'Control which summary cards appear at the top of the detail page and their order.',
  lineColumnsTitle = 'Line Items Columns',
  lineColumnsIntro = 'Control whether a line-item column shows and its default order.',
}: {
  detailHref: string
    ...
    <div className="space-y-6">
      {statCardDefinitions && layout.statCards ? (
        <div className="rounded-xl border p-6" style={{ backgroundColor: 'var(--card)', borderColor: 'var(--border-muted)' }}>
          ...
          <div className="space-y-2">
            {orderedStatCards.map((card, index) => (
              <div
                key={card.id}
                ...
                <div className="flex items-center gap-3">
                  <label className="flex items-center gap-2 text-xs" style={{ color: 'var(--text-secondary)' }}>
                    <input
                      ...
                      onChange={() => toggleStatCardVisible(card.id)}
                      className="h-4 w-4"
                    />
                    Show
                  </label>
                  <select
                    value={card.metric}
                    onChange={(event) => assignStatCardToSlot(card.id, event.target.value)}
                  ...
                  <button
                    type="button"
                    onClick={() => moveStatCard(card.id, -1)}
                  ...
                  <button
                    ...
                    disabled={layout.statCards!.length <= 1}
      ...
      {lineColumnDefinitions && layout.lineColumns ? (
        ...
        <div className="rounded-xl border p-4" style={{ backgroundColor: 'var(--card)', borderColor: 'var(--border-muted)' }}>
          {error ? <p className="text-sm" style={{ color: 'var(--danger)' }}>{error}</p> : <ActionButtons />}
        </div>
      )}
    </div>
  )
