...
type DragState = {
  fieldId: SalesOrderDetailFieldKey
  section: string
  column: number
  row: number
} | null

const SECTION_FIELD_IDS: Record<string, SalesOrderDetailFieldKey[]> = {
  Customer: [
    'customerName',
    'customerNumber',
    'customerEmail',
  ...
  'Sales Order Details': ['number', 'createdBy', 'createdFrom', 'opportunity', 'subsidiaryId', 'currencyId', 'status', 'total'],
}
...
export default function SalesOrderDetailCustomizeMode({
  ...
  const [layout, setLayout] = useState<SalesOrderDetailCustomizationConfig>(initialLayout)
  ...
  function resolveDragPayload(event?: React.DragEvent<HTMLElement>) {
    if (event) {
      const rawPayload = event.dataTransfer.getData('application/x-sales-order-field')
      if (rawPayload) {
        try {
  ...
  function moveFieldToCell(
    prev: SalesOrderDetailCustomizationConfig,
    fieldId: SalesOrderDetailFieldKey,
    nextSection: string,
    nextColumn: number,
    nextRow: number
  ...
  function handleDragStart(event: React.DragEvent<HTMLElement>, payload: DragState) {
    ...
    event.dataTransfer.setData('application/x-sales-order-field', JSON.stringify(payload))
  ...
  function toggleVisible(fieldId: SalesOrderDetailFieldKey) {
    setLayout((prev) => ({
      ...prev,
      fields: {
        ...prev.fields,
  ...
  function moveLineColumn(columnId: SalesOrderLineColumnKey, direction: -1 | 1) {
    setLayout((prev) => {
      const ordered = [...SALES_ORDER_LINE_COLUMNS]
        .map((column) => ({
          id: column.id,
  ...
  function toggleLineColumnVisible(columnId: SalesOrderLineColumnKey) {
    setLayout((prev) => ({
      ...prev,
      lineColumns: {
        ...prev.lineColumns,
  ...
  async function saveCustomization() {
    ...
    try {
      const response = await fetch('/api/config/sales-order-detail-customization', {
        method: 'POST',
      ...
      if (!response.ok) {
        setError(body?.error ?? 'Unable to save sales order customization')
        return
      }

      window.location.assign(detailHref)
    } catch {
      setError('Unable to save sales order customization')
    } finally {
      setSaving(false)
    }
  }

  const orderedLineColumns = [...SALES_ORDER_LINE_COLUMNS].sort(
  ...
  function getSectionFieldOptions(section: string) {
    ...
    return fieldIds
      ...
      .filter((field): field is { id: SalesOrderDetailFieldKey; label: string; placement: string } => Boolean(field))
  }
        ...
        <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm" style={{ color: 'var(--text-secondary)' }}>
              Edit the sales order header in context. Each filled box is a field placement, and empty boxes are open grid cells.
            </p>
          </div>
          <ActionButtons />
        </div>
                          ...
                          >
                            ...
                            {sectionFieldOptions.length > 0 ? (
                              <select
                                value=""
                                onChange={(event) => {
                                  const fieldId = event.target.value as SalesOrderDetailFieldKey
                                  if (!fieldId) return
