...
function createDraftContact(isPrimaryForCustomer = false): DraftContact {
  return {
...
export default function CustomerContactsSection({
  ...
  function addDraft() {
    setDrafts((previousDrafts) => [
      ...previousDrafts,
      createDraftContact(contacts.length === 0 && previousDrafts.length === 0),
    ])
  }
        ...
        <table className="min-w-full">
          <thead>
            <tr>
              ...
              <RecordDetailHeaderCell>Primary</RecordDetailHeaderCell>
          ...
          <tbody>
            {contacts.map((contact) => (
              <tr key={contact.id} id={`contact-${contact.id}`} tabIndex={-1} className="focus:outline-none transition-shadow">
                ...
                <RecordDetailCell>{contact.isPrimaryForCustomer ? 'Yes' : 'No'}</RecordDetailCell>
              ...
              <>
                ...
                <tr key={draft.id} style={{ borderTop: contacts.length > 0 || index > 0 ? '1px solid var(--border-muted)' : undefined }}>
                  ...
                  <RecordDetailCell className="text-center">
                    <input
                      type="checkbox"
                      checked={draft.isPrimaryForCustomer}
                      onChange={(event) => updateDraft(draft.id, { isPrimaryForCustomer: event.target.checked })}
                      className="h-4 w-4"
                    />
                  </RecordDetailCell>
                  <RecordDetailCell className="text-center">
