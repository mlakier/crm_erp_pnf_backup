...
import type { LocationFormCustomizationConfig, LocationFormFieldKey } from '@/lib/location-form-customization'
...
export default function LocationDetailCustomizeMode({
  ...
  const [layout, setLayout] = useState<LocationFormCustomizationConfig>(initialLayout)
  ...
  function resolveDragPayload(event?: React.DragEvent<HTMLElement>) {
    if (event) {
      const rawPayload = event.dataTransfer.getData('application/x-location-field')
      if (rawPayload) {
        try {
          const parsed = JSON.parse(rawPayload) as DragState
          if (parsed && typeof parsed.fieldId === 'string' && typeof parsed.section === 'string') return parsed
        } catch {
  ...
  function findFieldAtCell(config: LocationFormCustomizationConfig, section: string, column: number, row: number) {
  ...
  function moveFieldToCell(
    prev: LocationFormCustomizationConfig,
    fieldId: LocationFormFieldKey,
    nextSection: string,
    nextColumn: number,
    nextRow: number
  ...
  function handleDragStart(event: React.DragEvent<HTMLElement>, payload: DragState) {
    ...
    event.dataTransfer.setData('application/x-location-field', JSON.stringify(payload))
  ...
  function updateFormColumns(nextCount: number) {
    ...
    setLayout((prev) => ({
      ...
      ) as LocationFormCustomizationConfig['fields'],
    }))
  }
    ...
    setLayout((prev) => ({
      ...
      fields: Object.fromEntries(
        fields.map((field) => {
          ...
          return [field.id, { ...fieldConfig, order: Math.min(fieldConfig.order, rowCount - 1) }]
        })
      ) as LocationFormCustomizationConfig['fields'],
    }))
  }

  function toggleVisible(fieldId: LocationFormFieldKey) {
    setLayout((prev) => ({
      ...prev,
      fields: {
        ...prev.fields,
  ...
  function toggleRequired(fieldId: LocationFormFieldKey) {
  ...
  function deleteSection(sectionName: string) {
    ...
    setLayout((prev) => {
      ...
      return { ...prev, sections: nextSections, sectionRows: nextSectionRows }
    })
  ...
  async function saveCustomization() {
    ...
    try {
      const [layoutResponse, requirementsResponse] = await Promise.all([
        fetch('/api/config/location-form-customization', {
          method: 'POST',
        ...
        fetch('/api/config/form-requirements', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ config: { locationCreate: requirements } }),
        }),
      ])
      ...
      if (!layoutResponse.ok) {
        setError(layoutBody?.error ?? 'Unable to save location form layout')
        return
      }

      if (!requirementsResponse.ok) {
        setError(requirementsBody?.error ?? 'Unable to save location form requirements')
        return
      }

      window.location.assign(detailHref)
    } catch {
      setError('Unable to save location form customization')
    } finally {
      setSaving(false)
    }
  }
  ...
  function ActionButtons() {
    return (
      <div className="flex items-center gap-2">
        <Link href={detailHref} className="rounded-md border px-3 py-2 text-sm" style={{ borderColor: 'var(--border-muted)', color: 'var(--text-secondary)' }}>
          Cancel
        </Link>
        <button type="button" onClick={saveCustomization} disabled={saving} className="rounded-md px-3 py-2 text-sm font-semibold text-white disabled:opacity-60" style={{ backgroundColor: 'var(--accent-primary-strong)' }}>
    ...
    <div className="mb-8 rounded-xl border p-6" style={{ backgroundColor: 'var(--card)', borderColor: 'var(--border-muted)' }}>
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm" style={{ color: 'var(--text-secondary)' }}>
            Edit the real page layout directly. Empty boxes are open grid cells, and each filled box is the actual field placement.
          </p>
        </div>
        <ActionButtons />
      </div>
      ...
      <div className="mb-5 flex flex-wrap items-end gap-3">
        <label className="space-y-1 text-sm" style={{ color: 'var(--text-secondary)' }}>
          <span className="block text-xs uppercase tracking-wide" style={{ color: 'var(--text-muted)' }}>Form Columns</span>
          <select
        ...
        <label className="space-y-1 text-sm" style={{ color: 'var(--text-secondary)' }}>
          <span className="block text-xs uppercase tracking-wide" style={{ color: 'var(--text-muted)' }}>New Section</span>
          <input
      ...
      <div className="space-y-6">
        {layout.sections.map((section) => {
          ...
          const visibleFieldCount = fields.filter((field) => layout.fields[field.id].section === section && layout.fields[field.id].visible).length
                ...
                <div>
                  {editingSectionName === section ? (
                    <div className="flex flex-wrap items-center gap-2">
                      <input value={editingSectionValue} onChange={(event) => setEditingSectionValue(event.target.value)} className="rounded-md border bg-transparent px-2 py-1 text-sm text-white" style={{ borderColor: 'var(--border-muted)' }} />
                      <button type="button" onClick={() => saveSectionName(section)} className="rounded-md px-2 py-1 text-xs font-semibold text-white" style={{ backgroundColor: 'var(--accent-primary-strong)' }}>Save</button>
                      <button type="button" onClick={cancelEditingSection} className="rounded-md border px-2 py-1 text-xs" style={{ borderColor: 'var(--border-muted)', color: 'var(--text-secondary)' }}>Cancel</button>
                    </div>
                  ) : (
                    <div className="flex flex-wrap items-center gap-2">
                      <h3 className="text-sm font-semibold text-white">{section}</h3>
                      <button type="button" onClick={() => moveSection(section, -1)} className="rounded-md border px-2 py-1 text-xs" style={{ borderColor: 'var(--border-muted)', color: 'var(--text-secondary)' }}>Up</button>
                      <button type="button" onClick={() => moveSection(section, 1)} className="rounded-md border px-2 py-1 text-xs" style={{ borderColor: 'var(--border-muted)', color: 'var(--text-secondary)' }}>Down</button>
                      <button type="button" onClick={() => startEditingSection(section)} className="rounded-md border px-2 py-1 text-xs" style={{ borderColor: 'var(--border-muted)', color: 'var(--text-secondary)' }}>Edit</button>
                      <button type="button" onClick={() => deleteSection(section)} className="rounded-md border px-2 py-1 text-xs" style={{ borderColor: 'var(--danger)', color: 'var(--danger)' }}>Delete</button>
                    </div>
                  )}
                  {sectionDescriptions?.[section] ? <p className="mt-1 text-xs" style={{ color: 'var(--text-muted)' }}>{sectionDescriptions[section]}</p> : null}
                  <p className="mt-1 text-xs" style={{ color: 'var(--text-muted)' }}>{visibleFieldCount} visible fields</p>
                </div>
                <label className="space-y-1 text-sm" style={{ color: 'var(--text-secondary)' }}>
                  <span className="block text-xs uppercase tracking-wide" style={{ color: 'var(--text-muted)' }}>Section Rows</span>
                  <input
                    type="number"
                    min={1}
                    max={12}
              ...
              <div className="grid gap-3" style={{ gridTemplateColumns: `repeat(${layout.formColumns}, minmax(0, 1fr))` }}>
                {Array.from({ length: rowCount }, (_, rowIndex) =>
                  Array.from({ length: layout.formColumns }, (_, columnIndex) => {
                    ...
                    if (!occupant) {
                      return (
                        <div
                          ...
                          className="flex min-h-[5.5rem] items-center justify-center rounded-lg border border-dashed px-3 text-center text-xs"
                    ...
                    const dragPayload: DragState = { fieldId: field.id, section, column, row: rowIndex }
                      ...
                      >
                        <div className="mb-2 flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <div className="truncate text-xs font-medium" style={{ color: 'var(--text-muted)' }}>{field.label}</div>
                            <div className="mt-1 text-sm" style={{ color: 'var(--text-secondary)' }}>{field.previewValue || '-'}</div>
                          </div>
                          <div
                            draggable
                            onMouseDown={() => {
                        ...
                        <div className="mb-3 flex items-center gap-4">
                          <label className="flex items-center gap-2 text-xs" style={{ color: 'var(--text-secondary)' }}>
                            <input type="checkbox" checked={fieldConfig.visible} onChange={() => toggleVisible(field.id)} className="h-4 w-4" />
                            Show
                          </label>
                          <label className="flex items-center gap-2 text-xs" style={{ color: 'var(--text-secondary)' }}>
                            <input type="checkbox" checked={Boolean(requirements[field.id])} onChange={() => toggleRequired(field.id)} className="h-4 w-4" />
                            Required
                          </label>
                        </div>
                        ...
                        <div className="mt-auto pt-3 text-[11px]" style={{ color: 'var(--text-muted)' }}>
                          {fieldConfig.visible
                            ? `${field.fieldType} | ${section} | Column ${column} | Row ${row}`
