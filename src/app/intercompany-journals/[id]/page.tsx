...
export default async function IntercompanyJournalEntryDetailPage({
  params,
  searchParams,
}: {
  ...
  const systemNotes = activities
    .map((activity) => {
      ...
      return {
        ...
        oldValue: formatSystemNoteValue(parsed.fieldName, parsed.oldValue, currencyFields),
        newValue: formatSystemNoteValue(parsed.fieldName, parsed.newValue, currencyFields),
      }
    })
  ...
  return (
    <JournalEntryDetailClient
      mode="detail"
      editing={isEditing}
      detailHref={`/intercompany-journals/${entry.id}`}
      customizeHref={`/intercompany-journals/${entry.id}?customize=1`}
