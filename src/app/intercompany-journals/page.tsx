...
export default async function IntercompanyJournalsPage({
  searchParams,
}: {
  ...
  const buildPageHref = (p: number) => {
    ...
    s.set('page', String(p))
    return '/intercompany-journals?' + s.toString()
  }
  ...
  return (
    <div className="min-h-full px-8 py-8">
      <div className="mb-6 flex items-center justify-between gap-4">
        ...
        <div>
          <h1 className="text-xl font-semibold text-white">Intercompany Journals</h1>
          <p className="mt-1 text-sm" style={{ color: 'var(--text-secondary)' }}>{totalRows} total</p>
        </div>
        <Link
          href="/journals/intercompany/new"
        ...
        >
          <span className="mr-1.5 text-lg leading-none">+</span>New Intercompany Journal
        </Link>
      </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        {STATUS_OPTIONS.map((s) => {
          ...
          const href = '/intercompany-journals?' + new URLSearchParams({ ...(params.q ? { q: params.q } : {}), status: s, page: '1' }).toString()
      ...
      <section className="overflow-hidden rounded-2xl border" style={{ backgroundColor: 'var(--card)', borderColor: 'var(--border-muted)' }}>
        ...
        <div className="record-list-scroll-region overflow-x-auto" data-column-selector-table="intercompany-journals-list">
          <table className="min-w-full" id="intercompany-journals-list">
              ...
              {rows.length === 0 ? (
                ...
                <tr key={row.id} style={i < rows.length - 1 ? { borderBottom: '1px solid var(--border-muted)' } : {}}>
                  <td data-column="number" className="px-4 py-2 text-sm font-medium">
                    <Link href={`/intercompany-journals/${row.id}`} style={{ color: 'var(--accent-primary-strong)' }}>
                      {row.number}
                    </Link>
                  </td>
                  ...
                  <td data-column="actions" className="px-4 py-2 text-sm"><span className="flex items-center gap-2">
                    <Link
                      href={`/intercompany-journals/${row.id}?edit=1`}
