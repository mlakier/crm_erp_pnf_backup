  ...
  return (
    <div className="min-h-full px-8 py-8">
      ...
      <div className="mb-6 flex items-center gap-4 flex-wrap">
        ...
        <input value={selectedRoleId} onChange={e => handleRoleChange(e.target.value)} list="manage-permissions-role-options" className="rounded-md border bg-transparent px-3 py-2 text-sm text-white" style={{ borderColor: 'var(--border-muted)' }} />
        <datalist id="manage-permissions-role-options">
          {roles.map(role => <option key={role.id} value={role.id}>{role.name} ({role.roleId})</option>)}
        </datalist>
