...
export default async function ItemDetailPage({
  ...
  const [item, fieldOptions, itemFormCustomization, formRequirements] = await Promise.all([
  ...
  const itemFieldDefinitions: Record<ItemFormFieldKey, InlineRecordSection['fields'][number]> = {
    ...
    revenueStream: { name: 'revenueStream', label: 'Revenue Stream', value: item.revenueStream ?? '', type: 'select', options: fieldOptions.revenueStream ?? [], helpText: 'Business-facing revenue category used to group similar products or services for reporting and disclosure.', sourceText: getFieldSourceText(fieldMetaById, 'revenueStream') },
    recognitionMethod: {
    ...
    allocationEligible: { name: 'allocationEligible', label: 'Allocation Eligible', value: String(item.allocationEligible), type: 'checkbox', placeholder: 'Allocation Eligible', helpText: 'Controls whether this item participates in relative SSP contract allocation.' },
    performanceObligationType: {
    ...
    line: { name: 'line', label: 'Line', value: item.line ?? '', type: 'select', options: fieldOptions.line ?? [], helpText: 'Commercial or catalog line classification.', sourceText: getFieldSourceText(fieldMetaById, 'line') },
    ...
    dropShipItem: { name: 'dropShipItem', label: 'Drop Ship Item', value: String(item.dropShipItem), type: 'checkbox', placeholder: 'Drop Ship Item', helpText: 'Indicates this item is normally sourced via vendor direct shipment and can drive auto-PO behavior.' },
    specialOrderItem: { name: 'specialOrderItem', label: 'Special Order Item', value: String(item.specialOrderItem), type: 'checkbox', placeholder: 'Special Order Item', helpText: 'Indicates the item is specially procured and can be received through purchasing.' },
    ...
    inventoryAccountId: { name: 'inventoryAccountId', label: 'Asset Account', value: item.inventoryAccountId ?? '', type: 'select', placeholder: 'Select GL account', options: fieldOptions.inventoryAccountId ?? [], helpText: 'Asset account that carries inventory or capitalized item value.', sourceText: getFieldSourceText(fieldMetaById, 'inventoryAccountId') },
